...
export default function Usuarios() {
  ...
  const cargarEmpresas = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/usuarios/empresas", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (Array.isArray(data)) setEmpresas(data);
      else {
        console.warn("⚠️ No se devolvió lista de empresas:", data);
        setEmpresas([]);
      }
    } catch (err) {
  ...
  const cargarRoles = async () => {
    try {
      ...
      if (Array.isArray(data)) setRoles(data);
        ...
        console.warn("⚠️ No se devolvió lista de roles:", data);
        setRoles([]);
      }
    } catch (err) {
  ...
  const actualizarUsuario = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/usuarios/${form.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
