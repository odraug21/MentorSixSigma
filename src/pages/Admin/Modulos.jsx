...
export default function Modulos() {
  ...
  const [form, setForm] = useState({ nombre: "", tipo: "operativo", categoria: "General", descripcion: "" });
  const [editando, setEditando] = useState(null);
  const token = localStorage.getItem("token");

  const cargarModulos = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/modulos", {
...
const guardarModulo = async (e) => {
  ...
  try {
    if (editando) {
      await axios.put(`http://localhost:5000/api/modulos/${editando.id}`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } else {
      await axios.post("http://localhost:5000/api/modulos", form, {
        headers: { Authorization: `Bearer ${token}` },
      });
    }
    alert("✅ Módulo guardado correctamente");
    setForm({ nombre: "", tipo: "operativo", categoria: "General", descripcion: "" });
    setEditando(null);
    cargarModulos();
  } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <h1 className="text-3xl font-bold text-indigo-400 mb-6">Gestión de Módulos</h1>
...
>
  ...
  <div className="flex flex-col">
    <label className="text-indigo-300 font-semibold mb-1">Nombre del módulo</label>
    <input
      name="nombre"
      value={form.nombre}
...
{/* Categoría */}
<div className="flex flex-col">
  <label className="text-indigo-300 font-semibold mb-1">Categoría</label>

  {/* Si existen categorías, muestra select + opción de nueva */}
  {modulos.length > 0 && (
    <select
      name="categoria"
    ...
    >
      {/* Categorías únicas generadas dinámicamente */}
  ...
  {/* Campo para nueva categoría (solo visible si selecciona "nueva") */}
  {form.categoria === "" && (
    <input
      name="categoria"
      value={form.categoria}
  ...
  <div className="col-span-full flex flex-col">
    <label className="text-indigo-300 font-semibold mb-1">Descripción</label>
    <textarea
      name="descripcion"
    ...
    />
      ...
      <table className="w-full bg-gray-800 text-sm rounded-lg overflow-hidden border border-gray-700">
        ...
        <tbody>
          {modulos.map((m) => (
            <tr key={m.id} className="border-t border-gray-700 hover:bg-gray-800/70">
