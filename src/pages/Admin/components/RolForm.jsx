...
export default function RolForm({ rolEditando, onGuardar, onCancelar }) {
  ...
  const cargarModulosDelRol = async (rolId) => {
    if (!rolId) return;
    try {
      const res = await axios.get(`http://localhost:5000/api/roles-modulos/${rolId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rolRes = rol.id
        ? await axios.put(`http://localhost:5000/api/roles/${rol.id}`, rol, {
            headers: { Authorization: `Bearer ${token}` },
          })
        : await axios.post("http://localhost:5000/api/roles", rol, {
            headers: { Authorization: `Bearer ${token}` },
          });
      ...
      await Promise.all(
        modulos.map(async (m) => {
          ...
          await axios.post(
            "http://localhost:5000/api/roles-modulos",
            {
              rol_id: rolId,
              modulo_id: m.id,
              activo,
